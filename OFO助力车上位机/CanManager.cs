...
namespace KS5045上位机_只接收
{
    class CanManager
    {
        ...
        public void start()
        {
           init((UInt32)0, 8);//8 = 250k  10 = 500k
            ...
            config.Mode = (Byte)1;//模式 0 正常 1只听 2 自测
 ...
 //               can_id <<= 21;
        ...
        unsafe public Int32 StandardWrite(Byte[] write_buffer, byte cmd, byte len, int type)
        {
          

            VCI_CAN_OBJ sendobj = new VCI_CAN_OBJ();
            sendobj.RemoteFlag = (byte)0;//0：数据帧 1：远程帧
            sendobj.ExternFlag = (byte)1;//0：标准帧 1：扩展帧
            sendobj.TimeFlag = (byte)0;//1：使用时间标识 0：不使用
            sendobj.TimeStamp = Convert.ToUInt32(DateTime.Now.Second.ToString(), 16);
            sendobj.ID = Consts.MASTER_CAN_ID;
            sendobj.SendType = (byte)type;

 //           MessageBox.Show(System.Convert.ToString(sendobj.ID));
     //       MessageBox.Show(System.Convert.ToString(ID));
            sendobj.DataLen = System.Convert.ToByte(8);  //8
            for (int i = 0; i < 8; i++)   
            {
                sendobj.Data[i] = write_buffer[i];
